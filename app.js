...
function initDashboardPage() {
    ...
    const getPengaturan = async () => { /* ... (fungsi sama seperti sebelumnya) ... */ };
    const getPemenangTerakhir = async () => {
        const q = query(collection(db, "peserta"), where("status_menang", "==", true), limit(1));
        const querySnapshot = await getDocs(q);
        if (!querySnapshot.empty) return querySnapshot.docs[0].data().nama;
        return "Belum ada pemenang";
    };

    const loadDashboardData = async () => {
        try {
            const [pengaturan, pesertaSnap, pemenangTerakhir] = await Promise.all([
                getPengaturan(),
            ...
            // ... (sisa logika load data dashboard tidak berubah) ...
        } catch (error) {
            console.error("Gagal memuat data dashboard:", error);
            // ... (sisa logika error tidak berubah) ...
            pemenangTerakhirEl.textContent = 'Error';
        }
    };
    loadDashboardData();

    // ... (sisa logika kocokan tidak berubah) ...
}
    ...
    const loadPeserta = async () => {
        ...
        snap.forEach(d => {
            ...
            tr.innerHTML = `
                ...
                <td class="p-4"><div class="flex items-center justify-end gap-2"><button class="edit-btn ..."><i ...></i></button><button class="delete-btn ..."><i ...></i></button></div></td>
            `;
...
function initPengaturanPage() {
    // ... (Logika halaman pengaturan tidak berubah) ...
}